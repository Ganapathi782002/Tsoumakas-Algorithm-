...
'''Each classifier implements a fit method to train the model on the training data (x_mat, y_mat), 
a predict_proba method to predict the probabilities of each label, 
and a predict method to predict the labels for new samples.'''

#x_mat = Matrix where (each row = samples and each columns = features)
#y_mat = Matrix where (each row = samples and each columns = respective labels)
class OVAMultiLabelClassifier: #One-Vs-All classifier
...
class BinaryLinearSVCWrapper(LinearSVC): #enhances linear Support Vector Classification using sigmoid function
