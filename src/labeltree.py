...
#At each node, the label space and data points are partitioned based on the labels.
'''The splitting process continues until a stopping condition is met, 
which is usually based on the size of the label space or the data points.'''
#Classifiers are then trained at each node based on the partitioned data.
#During prediction, probabilities are calculated recursively starting from the root node down to the leaf nodes.
#It uses BEAM SEARCH HEURISTICS ALGORITHM.
#This file has the predict_proba function for predicting labels.
#The stopping condition is when the number of labels assigned to a particular node falls below a specified threshold(here it is 0.5)
...
class LabelTree:
	...
	def _fit_classifiers(self,x_mat,y_mat):
		...
		#shows how many classifiers are being trained simultaneously or successively.
...
class LabelTreeNode:
	...
	def _split(self,x_mat,y_mat,repre):
		...
		# partition labels and filter empty partitions
